class Employee {
  ...
  formatDetails(): string {
    return `${this.name} ${this.salary.toFixed(2)} ${this.email || "n/a"} ${
      this.age || -1
    }`;
  }
}
...
// Main program
function companyRoster(input: string[]): void {
  const departments: { [name: string]: Department } = {};
  ...
  let highestAvgDepartment: Department | null = null;
